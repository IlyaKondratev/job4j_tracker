...
public class SearchAttTest {
    ...
    public void testFilterTwo() {
        ...
        List<Attachment> filterNameList = SearchAtt.filter(list, att -> att.getName().contains("bug"));
        List<Attachment> expect1 = Arrays.asList(new Attachment("debug", 10));
        assertThat(filterNameList, is(expect1));
        List<Attachment> filterSizeList = SearchAtt.filter(list, att -> att.getSize() > 100);
